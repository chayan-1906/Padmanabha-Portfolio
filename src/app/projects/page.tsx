...
async function ProjectWrapper() {
    ...
    projects.forEach((project: Project) => {
        ...
        const categoryKey: string = project.projectCategory.title?.toLowerCase();
    ...
    const sortedCategories: Record<string, GroupedProjectCategory> = Object.entries(groupedProjectsMap)
    ...
    return (
        <>
            <AnalyticsTracker pageUrl={'/projects'}/>
            ...
            <main className={cn('min-h-screen pt-32 pb-20 px-6')} style={{backgroundColor: 'rgb(var(--color-background))'}}>
                <div className={cn('max-w-7xl mx-auto')}>
                    ...
                    <div className={cn('text-center mb-20')}>
                        <h1 className={cn('text-5xl md:text-6xl font-bold mb-6 leading-16 md:leading-24 bg-linear-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent')}>
                            All Projects
                        </h1>
